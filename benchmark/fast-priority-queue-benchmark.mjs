
import Benchmark from "./benchmark.mjs";
import FastPriorityQueue from "fastpriorityqueue";

export default class FastPriorityQueueBenchmark extends Benchmark {

    constructor(...args) {
        super("FastPQ", ...args);
        this.reset();
    }

    reset() {
        this.q = new FastPriorityQueue();
    }

    buildTest(indexes, data) {
        this.q.heapify(Array.from(data));
    }

    pushTest(data) {
        for (let i = 0; i < this.numberOfKeys; i++) {
            this.q.add(data[i]);
        }
    }

    popTest() {
        for (let i = 0; i < this.numberOfKeys; i++) {
            this.q.poll();
        }
    }

    pushPopBatchTest(data) {
        for (let i = 0; i < this.numberOfKeys; i += this.batchSize) {
            for (let j = 0; j < this.batchSize; j++) {
                this.q.add(data[i + j]);
            }
            for (let j = 0; j < this.batchSize; j++) {
                this.q.poll();
            }
        }
    }

    preparePushPopInterleaved(data, prepareSize) {
        for (let i = 0; i < prepareSize; i++) {
            this.q.add(data[i]);
        }
    }

    pushPopInterleaved(data, prepareSize, remainingSize) {
        for (let i = prepareSize; i < remainingSize; i++) {
            this.q.add(data[i]);
            this.q.poll();
        }
    }

    preparePushPopRandom(data, prepareSize) {
        // add a few before starting
        for (let i = 0; i < prepareSize; i++) {
            this.q.add(data[i]);
        }
        const remainingSize = this.numberOfKeys - prepareSize;

        const walk = [];
        const pop = this.q.poll.bind(this.q);
        for (let i = prepareSize; i < remainingSize; i++) {
            walk.push(Math.random() < 0.5 ? this.q.add.bind(this.q, data[i]) : pop);
        }
        return walk;
    }

    pushPopRandom(walk) {
        for (let i = 0; i < walk.length; i++) {
            walk[i]();
        }
    }
}
